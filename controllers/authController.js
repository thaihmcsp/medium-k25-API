...
exports.register = async (req, res) => {
    try {
        ...
        if(checkUser) return res.status(400).json({message: 'email is used, choose another one'});

        const password = bcrypt.hashSync(req.body.password, 10);
        await User.create({email: req.body.email, username: req.body.username, password});
